...
async function fetchContributionYear(username: string, year: number): Promise<GitHubContributionDay[]> {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`GitHub calendar returned status ${response.status}`);

    const days = parseGitHubContributionCalendar(await response.text());
    if (days.length === 0) throw new Error('GitHub returned an unreadable contribution calendar');
    return days;
  } finally {
...
export async function getGitHubContributions(username: string, today = new Date()): Promise<GitHubContributionDay[]> {
  if (!GITHUB_USERNAME_PATTERN.test(username)) throw new Error('Invalid GitHub username');
